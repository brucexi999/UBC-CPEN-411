...
VOID Instruction(INS ins, VOID *v)
{
	...
	// Check if the instruciton is a memory instruction
	if(INS_IsMemoryRead(ins) || INS_IsMemoryWrite(ins)){
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)MemAccess, IARG_END);
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)serialDelay, IARG_BOOL, true, IARG_END);
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)hideMemDelay, IARG_BOOL, true, IARG_END);
	}else{
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)NonMemAccess, IARG_END);
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)serialDelay, IARG_BOOL, false, IARG_END);
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)hideMemDelay, IARG_BOOL, false, IARG_END);
	}

	// Check if the instruction is a branch instruction
	if(INS_IsBranch(ins)){
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)branch, IARG_END);
	}
